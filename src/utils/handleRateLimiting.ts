	...
	if (bypassRoles.some(role => interaction.member.roles.cache.has(role))) {
		...
		// Postgres Database Selected
			...
			if (nonce) {
				...
				if (nonceObj) {
					const lastReqTime = nonceObj.unlockTime.getMilliseconds();
			...
			if (user) {
				const lastReqTime = user.unlockTime.getMilliseconds();
				if (Date.now() - lastReqTime < stats.coolDownTime) {
					return lastReqTime;
				}
			}
			// Return the time if there is an amount remaining
		}
		return undefined;
	}
};
...
module.exports = { getTime, setTimer };
